    ...
    public class PoseNet1 : BaseImagePredictor<float>
    {
        ...
        public Result[] GetResults()
        {
            ...
            for (int part = 0; part < results.Length; part++)
            {
                ...
                {
                    if (res.confidence > 0.01)//confidence 값에 따라 보이는 값이 달라짐 0~1사이(화면에 나타나면 1에 가깝게 나옴)
                    {
                        ...
                        if ((0.3 < res.x && res.x < 0.8) && (0.5 < res.y && res.y < 0.9))
                        {
                            
                            if ((-0.04 < differentx && differentx < 0.04) && (-0.04 < differenty && differenty < 0.04))//손 안움직일 때
                            {
