    ...
    public class PoseNet : BaseImagePredictor<float>
    {
        ...
        public Result[] GetResults()
        {
            ...
            for (int part = 0; part < results.Length; part++)
            {
                ...
                {
                    if (res.confidence > 0.005)//confidence 값에 따라 보이는 값이 달라짐 0~1사이(화면에 나타나면 1에 가깝게 나옴)
                    {
                        ...
                        if ((0.2 < res.x && res.x < 1.0) && (0.2 < res.y && res.y < 1.1))//입 주위
                        {
                            if ((-0.02 < differentx && differentx < 0.02) && (-0.02 < differenty && differenty < 0.02))//손 안움직일 때
                            {
